...
const Admin = () => {
  ...
  const getAdminsHandler = useCallback(async (page = 1) => {
    try {
      setDataLoading(true);
      const response = await axiosWrapper.get(`/admin?page=${page}&limit=10`, {
        headers: {
          Authorization: `Bearer ${userToken}`,
        },
      });
      if (response.data.success) {
        const adminList = response.data.data?.admins || response.data.data || [];
  ...
  const searchAdmins = async (e, page = 1) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Error searching admins");
    } finally {
      setDataLoading(false);
    }
  };
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages) {
      if (hasSearched) {
        searchAdmins(null, newPage);
      } else {
        getAdminsHandler(newPage);
      }
    }
  };
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                <div>
                  ...
                  <input type="file" onChange={(e) => setFile(e.target.files[0])} className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" accept="image/*" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Blood Group</label>
                   <select value={data.bloodGroup} onChange={(e) => handleInputChange("bloodGroup", e.target.value)} className="w-full px-4 py-2 border rounded-md" required>
                ...
                <div>
                  ...
                  <input type="date" value={data.joiningDate} onChange={(e) => handleInputChange("joiningDate", e.target.value)} className="w-full px-4 py-2 border rounded-md" required />
                </div>
                <div>
              ...
              <div className="mt-8 flex justify-between items-center gap-4">
                <p className="text-sm">Default password: <span className="font-bold">admin123</span></p>
                <div className="flex gap-4">
                  <CustomButton type="button" variant="secondary" onClick={resetForm}>Cancel</CustomButton>
                  <CustomButton type="submit" variant="primary">{isEditing ? "Update Admin" : "Add Admin"}</CustomButton>
                </div>
              </div>
            </form>
          </div>
      ...
      {!dataLoading && !showAddForm && (
        <div className="w-full mt-5">
          ...
          <div className="bg-[#111827] p-6 rounded-lg border border-[#1F2937] mb-8">
            <h3 className="text-white text-lg font-medium mb-4">Filter Admins</h3>
            <form onSubmit={searchAdmins} className="flex flex-wrap gap-4">
              <input
                type="text"
                placeholder="Search by Name"
                className="flex-1 min-w-[200px] bg-[#1F2937] border border-[#374151] rounded-lg px-4 py-2 text-white outline-none focus:border-purple-500 transition-colors"
              ...
              />
              <CustomButton type="submit" variant="primary" className="px-8">Search</CustomButton>
              {hasSearched && (
                <button type="button" onClick={() => { setSearchParams({ name: "", email: "" }); setHasSearched(false); getAdminsHandler(1); }} className="text-gray-400 hover:text-white transition-colors">Clear</button>
              )}
            </form>
          </div>
                    ...
                    <tr key={index} className="hover:bg-[#1F2937]/50 text-[#9CA3AF]">
                      ...
                      <td className="py-4 px-6 text-center">
                        <div className="flex justify-center gap-3">
                          ...
                          <button onClick={() => editAdminHandler(item)} className="p-2 bg-purple-500/10 text-purple-500 rounded-lg hover:bg-purple-500 hover:text-white transition-all"><MdEdit size={18} /></button>
                          <button onClick={() => deleteAdminHandler(item._id)} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all"><MdOutlineDelete size={18} /></button>
                        </div>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan="6" className="text-center py-10 text-gray-500">No admins found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center mt-6 gap-3">
              <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="px-4 py-2 rounded-lg bg-[#1F2937] text-[#F3F4F6] border border-[#374151] disabled:opacity-50 hover:bg-[#374151] transition-all">Previous</button>
              <span className="text-[#9CA3AF]">Page <span className="text-[#F3F4F6] font-medium">{currentPage}</span> of {totalPages}</span>
              <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-4 py-2 rounded-lg bg-[#1F2937] text-[#F3F4F6] border border-[#374151] disabled:opacity-50 hover:bg-[#374151] transition-all">Next</button>
            </div>
          )}
        </div>
      )}
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <p><strong>Status:</strong> <span className={`px-2 py-1 rounded text-xs ${viewData.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{viewData.status}</span></p>
                </div>
                
                <div className="border-t pt-4">
                  <h3 className="font-semibold mb-2">Location:</h3>
                  <p>{viewData.address}</p>
                  <p>{viewData.city}, {viewData.state} - {viewData.pincode}</p>
